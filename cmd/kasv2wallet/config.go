...
type createConfig struct {
	KeysFile          string `long:"keys-file" short:"f" description:"Keys file location (default: ~/.kaspav2wallet/keys.json (*nix), %USERPROFILE%\\AppData\\Local\\kaspav2wallet\\key.json (Windows))"`
...
type sendConfig struct {
	KeysFile                 string   `long:"keys-file" short:"f" description:"Keys file location (default: ~/.v2wallet/keys.json (*nix), %USERPROFILE%\\AppData\\Local\\v2wallet\\key.json (Windows))"`
	Password                 string   `long:"password" short:"p" description:"Wallet password"`
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send KaspaV2 to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send KaspaV2 from. Repeat multiple times (adding -a before each) to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in KaspaV2 (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the KaspaV2 in the wallet (mutually exclusive with --send-amount). If --from-address was used, will send all only from the specified addresses."`
...
type createUnsignedTransactionConfig struct {
	DaemonAddress            string   `long:"daemonaddress" short:"d" description:"Wallet daemon server to connect to"`
	ToAddress                string   `long:"to-address" short:"t" description:"The public address to send KaspaV2 to" required:"true"`
	FromAddresses            []string `long:"from-address" short:"a" description:"Specific public address to send KaspaV2 from. Use multiple times to accept several addresses" required:"false"`
	SendAmount               string   `long:"send-amount" short:"v" description:"An amount to send in KaspaV2 (e.g. 1234.12345678)"`
	IsSendAll                bool     `long:"send-all" description:"Send all the KaspaV2 in the wallet (mutually exclusive with --send-amount)"`
	UseExistingChangeAddress bool     `long:"use-existing-change-address" short:"u" description:"Will use an existing change address (in case no change address was ever used, it will use a new one)"`
	config.NetworkFlags
}
...
type startDaemonConfig struct {
	...
	Listen    string `long:"listen" short:"l" description:"Address to listen on (default: 0.0.0.0:8281)"`
...
func parseCommandLine() (subCommand string, config interface{}) {
	...
	parser.AddCommand(balanceSubCmd, "Shows the balance of a public address",
		"Shows the balance for a public address in KaspaV2", balanceConf)

	sendConf := &sendConfig{DaemonAddress: defaultListen}
	parser.AddCommand(sendSubCmd, "Sends a KaspaV2 transaction to a public address",
		"Sends a KaspaV2 transaction to a public address", sendConf)
	...
	parser.AddCommand(createUnsignedTransactionSubCmd, "Create an unsigned KaspaV2 transaction",
		"Create an unsigned KaspaV2 transaction", createUnsignedTransactionConf)
